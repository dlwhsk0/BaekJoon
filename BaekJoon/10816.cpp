/* lower_bound / upper_bound 이분탐색 응용 알고리즘
lower_bound : 정렬된 수들 중 k 이상의 최초값 index
upper_bound : 정렬된 수들 중 k 초과의 최초값 index
이분 탐색과의 차이 : 일치하는 값이 아닌 이상의 값을 찾는 것이기 때문에 항상 결과가 존재한다
*/
